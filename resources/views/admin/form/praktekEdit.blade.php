...
<div class="container">
    <x-layouts.navbarAdmin>
        <x-slot name="mainContent">
            <x-layouts.bannerAdmin icon="bi bi-pencil-square" title="Kemahasiswaan" subTitle="Formulir Edit Seminar Kerja Praktek"/>
                ...
                <div class="col-12 col-lg-10">
                    <div class="card bg-white shadow rounded-4 overflow-hidden">
                        <div class="card-body p-4 p-lg-5">
                            ...
                            <form id="form" method="POST" action="/practices/{{ $practice->id }}">
                                @csrf
                                @method('PUT')
                                ...
                                <div class="mb-4">
                                    <label class="form-label fw-bold" style="color: #11667B;">Judul <span class="text-danger">*</span></label>
                                    <textarea class="form-control bg-light border border-secondary-subtle shadow-none @error('title') is-invalid @enderror" 
                                              name="title" 
                                              rows="4" 
                                              placeholder="Tuliskan judul secara lengkap..."
                                              style="border-radius: 10px;">{{ old('title', $practice->title) }}</textarea>
                                    @error('title')
                                        <div class="invalid-feedback fw-semibold">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="mb-4 border-bottom pb-4">
                                    <label class="form-label fw-bold" style="color: #11667B;">Pembimbing / Penguji <span class="text-danger">*</span></label>
                                    <input type="text" 
                                           class="form-control bg-light border border-secondary-subtle shadow-none @error('supervisor') is-invalid @enderror" 
                                           name="supervisor" 
                                           value="{{ old('supervisor', $practice->supervisor) }}"
                                           placeholder="Tuliskan nama beserta gelar pembimbing/penguji..."
                                           style="border-radius: 10px;">
                                    @error('supervisor')
                                        <div class="invalid-feedback fw-semibold">{{ $message }}</div>
                                    @enderror
