<div id="table" class="container">
    ...
    <div class="card bg-white shadow rounded-4 overflow-hidden">
        ...
        <div class="card-header bg-white border-bottom p-4 d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-2">
            ...
            <a href="/theses/create" class="btn btn-primary fw-semibold px-4 rounded-pill shadow-sm text-nowrap" style="background-color: #11667B; border-color: #11667B;">
                        ...
                        @forelse ($theses as $thesis)
                            <tr>
                                <td class="text-center fw-semibold text-secondary">
                                    {{ ($theses->currentPage() - 1) * $theses->perPage() + $loop->iteration }}
                                </td>
                                <td>
                                    <span class="fw-bold text-dark">{{ $thesis->name }}</span>
                                </td>
                                <td>
                                    <span class="text-secondary fw-semibold">{{ $thesis->nim }}</span>
                                </td>
                                <td class="text-center">
                                    <span class="text-dark fw-medium">
                                        {{ \Carbon\Carbon::parse($thesis->date)->locale('id')->translatedFormat('l, d F Y') }}
                                    </span>
                                </td>
                                <td class="text-center">
                                    <span class="fs-6 badge bg-warning bg-opacity-10 text-dark border border-warning-subtle px-2 py-1 rounded-pill">
                                        <i class="bi bi-clock me-1 text-warning"></i> {{ \Carbon\Carbon::parse($thesis->date)->locale('id')->translatedFormat('H:i') }} WIB
                                    </span>
                                </td>
                                <td class="text-center">
                                ...
                                <td>
                                    <div class="d-flex justify-content-center gap-2">
                                        <a href="/theses/{{ $thesis->id }}/edit" class="btn btn-sm btn-outline-warning border-warning-subtle text-dark shadow-sm" title="Edit Data">
                                            <i class="bi bi-pencil-square"></i>
                                        </a>
                                        <x-layouts.deleteAdminBtn data="/theses/{{ $thesis->id }}"/>
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="7" class="text-center py-5 text-muted">
                                    ...
                                    Data jadwal sidang skripsi belum tersedia atau tidak ditemukan.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
        ...
        <div class="card-footer bg-white border-top p-4" wire:loading.class="opacity-50" wire:target="gotoPage, previousPage, nextPage, search">
            {{ $theses->links(data: ['scrollTo' => '#table']) }}
        </div>
