<div id="table" class="container">
    ...
    <div class="card bg-white shadow rounded-4 overflow-hidden">
        ...
        <div class="card-header bg-white border-bottom p-4 d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3">
            ...
            <div class="d-flex flex-column flex-md-row gap-2" style="flex-grow: 1; max-width: 600px;">
                <div class="input-group">
                    ...
                    <input wire:model.live.debounce.250ms="search" 
                           ...
                           placeholder="Cari Judul Riset / Nama Tim..." 
                           aria-label="Search" />
                </div>
            ...
            <a href="/researchs/create" class="btn btn-primary fw-semibold px-4 rounded-pill shadow-sm text-nowrap" style="background-color: #11667B; border-color: #11667B;">
                ...
                <table class="table table-hover align-middle mb-0">
                    <thead class="table-light">
                        <tr>
                            ...
                            <th class="text-muted" style="width: 45%;">Judul Riset</th>
                            <th class="text-muted" style="width: 25%;">Tim Riset</th>
                        ...
                        @forelse ($researchs as $research)
                            <tr>
                                <td class="text-center fw-semibold text-secondary">
                                    {{ ($researchs->currentPage() - 1) * $researchs->perPage() + $loop->iteration }}
                                </td>
                                <td>
                                    <span class="fw-bold text-dark">{{ $research->title }}</span>
                                </td>
                                <td>
                                    <span class="text-secondary">{{ $research->team }}</span>
                                </td>
                                ...
                                <td>
                                    <div class="d-flex justify-content-center gap-2">
                                        <a href="/researchs/{{ $research->id }}/edit" class="btn btn-sm btn-outline-warning border-warning-subtle text-dark shadow-sm" title="Edit Data">
                                            <i class="bi bi-pencil-square"></i>
                                        </a>
                                        <x-layouts.deleteAdminBtn data="/researchs/{{ $research->id }}"/>
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="5" class="text-center py-5 text-muted">
                                    <i class="bi bi-flask fs-2 d-block mb-2 text-muted opacity-50"></i>
                                    Data riset dosen belum tersedia atau tidak ditemukan.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
