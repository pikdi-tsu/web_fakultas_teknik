        ...
        <table class="table table-hover">
            <thead>
                <tr>
                    ...
                    <th class="bg-white" scope="col">Judul Skripsi</th>
            ...
            <tbody>
                @foreach ($theses as $thesis)
                    <tr>
                        <td class="bg-white text-center" scope="row">{{ ($theses->currentPage() - 1) * $theses->perPage() + $loop->iteration }}</td>
                        <td class="bg-white" style="width: 25%">{{ $thesis->name  }}</td>
                        <td class="bg-white">{{ $thesis->nim  }}</td>
                        <td class="bg-white" style="width: 40%">{{ $thesis->title  }}</td>
                        <td class="bg-white"><a href="/jadwalSidang/{{ $thesis->id }}" type="button" class="btn btn-info fw-bold text-white w-100">Lihat Jadwal</a></td>
                    </tr>
                @endforeach
            </tbody>
        </table>
